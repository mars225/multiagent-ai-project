...
# Lancer backend + frontend en parallèle (adapter les commandes selon CLAUDE.md)
npm run dev   # ou yarn dev, ou autre commande de CLAUDE.md
```
...
### `.env.example` (adapter selon les variables de CLAUDE.md)
```env
DB_HOST=localhost
DB_PORT=[port]
DB_USERNAME=[user]
...
[commande dev du projet — voir CLAUDE.md]
