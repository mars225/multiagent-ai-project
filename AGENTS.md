...
## Agents disponibles (`.codex/agents/`)

Les agents sont des sous-agents spécialisés invocables via le tool `Agent`. Chaque fichier `.md` dans `.codex/agents/` définit un rôle, ses responsabilités et ses règles de travail. **Tous les agents lisent `AGENTS.md` au démarrage** pour connaître le stack et les conventions du projet courant.
...
| Agent | Fichier | Rôle | Déclencher quand… |
|-------|---------|------|-------------------|
| **Codeur** | `codeur.md` | Implémente les fonctionnalités en respectant les conventions AGENTS.md | Implémentation d'une feature après planification |
...
> **Note** : Les agents lisent `AGENTS.md` au démarrage pour connaître le stack et les conventions. Toujours fournir les chemins de fichiers et le contexte métier dans le prompt — ils ne voient pas la conversation précédente.
