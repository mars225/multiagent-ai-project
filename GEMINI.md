...
## Agents disponibles (`.gemini/agents/`)

Les agents sont des sous-agents spécialisés invocables via le tool `Agent`. Chaque fichier `.md` dans `.gemini/agents/` définit un rôle, ses responsabilités et ses règles de travail. **Tous les agents lisent `GEMINI.md` au démarrage** pour connaître le stack et les conventions du projet courant.
...
| Agent | Fichier | Rôle | Déclencher quand… |
|-------|---------|------|-------------------|
| **Codeur** | `codeur.md` | Implémente les fonctionnalités en respectant les conventions GEMINI.md | Implémentation d'une feature après planification |
...
Agent({
  prompt: "Ajoute un endpoint PATCH /api/[ressource]/:id/[action].
           ...
           Respecter les conventions de GEMINI.md."
})
```

> **Note** : Les agents lisent `GEMINI.md` au démarrage pour connaître le stack et les conventions. Toujours fournir les chemins de fichiers et le contexte métier dans le prompt — ils ne voient pas la conversation précédente.
